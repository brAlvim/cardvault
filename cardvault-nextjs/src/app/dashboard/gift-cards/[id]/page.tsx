...
export default function GiftCardDetails({ params }: { params: { id: string } }) {
  const [sidebarOpen, setSidebarOpen] = useState(true);
  
  // Mock data for a specific gift card
  const giftCard = {
    id: params.id || 'GC001',
    codigo: params.id || 'GC001',
    fornecedor: 'Amazon',
    valorInicial: 100,
    saldoAtual: 75,
    status: 'Ativo',
    ...
    observacoes: 'Gift card para compras de livros e eletrônicos.'
  };

  // Mock data for transactions
  const transactions = [
    { id: 'T001', data: '12/03/2025', valor: 10, usuario: 'Maria Costa', descricao: 'Compra de livro', status: 'Concluída' },
    { id: 'T002', data: '11/03/2025', valor: 15, usuario: 'João Silva', descricao: 'Assinatura mensal', status: 'Concluída' },
    { id: 'T003', data: '10/03/2025', valor: 0, usuario: 'João Silva', descricao: 'Cadastro inicial', status: 'Concluída' },
  ];
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6 bg-gray-100">
          <div className="flex items-center mb-6">
            ...
            <h1 className="text-2xl font-bold text-gray-800">Detalhes do Gift Card</h1>
          </div>
          ...
          <div className="bg-white rounded-lg shadow mb-6">
              ...
              <div className="flex justify-between items-start">
                <div>
                  <h2 className="text-xl font-semibold text-gray-800 mb-2">
                    {giftCard.fornecedor} - {giftCard.codigo}
                  </h2>
                  <div className="flex items-center">
                    <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                      giftCard.status === 'Ativo' ? 'bg-green-100 text-green-800' : 
                      giftCard.status === 'Zerado' ? 'bg-gray-100 text-gray-800' : 
                ...
                <div className="flex space-x-2">
                  ...
                  <button className="flex items-center px-3 py-2 border border-red-300 rounded-md text-sm text-red-700 hover:bg-red-50">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div>
                  <p className="text-sm font-medium text-gray-500">Valor Inicial</p>
                  <p className="mt-1 text-lg font-semibold text-gray-900">R$ {giftCard.valorInicial.toFixed(2)}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-500">Saldo Atual</p>
                  <p className="mt-1 text-lg font-semibold text-gray-900">R$ {giftCard.saldoAtual.toFixed(2)}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-500">Data de Validade</p>
                  <p className="mt-1 text-lg font-semibold text-gray-900">{giftCard.dataValidade}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-500">Data de Cadastro</p>
                  <p className="mt-1 text-lg font-semibold text-gray-900">{giftCard.dataCadastro}</p>
                </div>
              </div>
              <div className="mt-6">
                <p className="text-sm font-medium text-gray-500">Criado por</p>
                <p className="mt-1 text-lg font-semibold text-gray-900">{giftCard.criadoPor}</p>
              </div>
              <div className="mt-6">

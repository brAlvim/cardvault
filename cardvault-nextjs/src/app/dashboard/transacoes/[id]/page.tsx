...
export default function TransactionDetails({ params }: { params: { id: string } }) {
  ...
  const transaction = {
    ...
    saldoResultante: 90,
    usuario: 'Maria Costa',
    descricao: 'Compra de livro digital "Clean Code" na Amazon',
    status: 'Concluída',
    comprovante: 'https://example.com/receipt/12345',
    observacoes: 'Transação realizada via aplicativo móvel.'
  };
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6 bg-gray-100">
          <div className="flex items-center mb-6">
            <Link href="/dashboard/transacoes" className="flex items-center text-blue-600 hover:text-blue-800 mr-4">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
              </svg>
              <span>Voltar para Transações</span>
            </Link>
            <h1 className="text-2xl font-bold text-gray-800">Detalhes da Transação</h1>
          </div>
          ...
          <div className="bg-white rounded-lg shadow mb-6">
                ...
                <div>
                  ...
                  <div className="flex items-center">
                    <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                      transaction.status === 'Concluída' ? 'bg-green-100 text-green-800' : 
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                <div>
                  <p className="text-sm font-medium text-gray-500">Gift Card</p>
                  <p className="mt-1 text-lg font-semibold text-blue-600">
                    <Link href={`/dashboard/gift-cards/${transaction.giftCard}`}>
                ...
                <div>
                  ...
                  <p className="mt-1 text-lg font-semibold text-gray-900">{transaction.fornecedor}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-500">Data e Hora</p>
                  <p className="mt-1 text-lg font-semibold text-gray-900">{transaction.data} às {transaction.hora}</p>
                </div>
              </div>
              
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div>
                  <p className="text-sm font-medium text-gray-500">Valor Utilizado</p>
                  <p className="mt-1 text-lg font-semibold text-red-600">R$ {transaction.valorUtilizado.toFixed(2)}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-500">Saldo Anterior</p>
                  <p className="mt-1 text-lg font-semibold text-gray-900">R$ {transaction.valorInicial.toFixed(2)}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-500">Saldo Result<response clipped><NOTE>To save on context only part of this file has been shown to you. You should retry this tool after you have searched inside the file with `grep -n` in order to find the line numbers of what you are looking for.</NOTE>
...
export default function TransactionForm() {
  ...
  const giftCards = [
    { id: 'GC001', fornecedor: 'Amazon', saldo: 75 },
    { id: 'GC002', fornecedor: 'Netflix', saldo: 50 },
    { id: 'GC003', fornecedor: 'Spotify', saldo: 25 },
    { id: 'GC004', fornecedor: 'Steam', saldo: 150 },
    { id: 'GC005', fornecedor: 'Uber', saldo: 30 },
  ];
  ...
  // Get selected gift card details
  const selectedGiftCard = giftCards.find(card => card.id === formData.giftCard);
        ...
        <main className="flex-1 overflow-y-auto p-6 bg-gray-100">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <form onSubmit={handleSubmit}>
              <div className="mb-6">
                <label htmlFor="giftCard" className="block text-sm font-medium text-gray-700 mb-1">
                  Gift Card *
                </label>
                <select
                  id="giftCard"
                ...
                >
                  <option value="">Selecione um gift card</option>
                  {giftCards.map((card) => (
                    <option key={card.id} value={card.id}>{card.id} - {card.fornecedor} (Saldo: R$ {card.saldo.toFixed(2)})</option>
                  ))}
                </select>
              </div>
              ...
              {selectedGiftCard && (
                <div className="mb-6 p-4 bg-blue-50 rounded-md">
                  <h3 className="text-md font-medium text-gray-700 mb-2">Informações do Gift Card</h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <p className="text-sm font-medium text-gray-500">Código</p>
                      <p className="text-sm font-semibold">{selectedGiftCard.id}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-500">Fornecedor</p>
                      <p className="text-sm font-semibold">{selectedGiftCard.fornecedor}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-500">Saldo Disponível</p>
                      <p className="text-sm font-semibold">R$ {selectedGiftCard.saldo.toFixed(2)}</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="mb-6">
                ...
                <input
                  ...
                  min="0.01"
                  step="0.01"
                  max={selectedGiftCard ? selectedGiftCard.saldo : undefined}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                  placeholder="Ex: 10.00"
                />
                {selectedGiftCard && (
                  <p className="mt-1 text-sm text-gray-500">
                    Saldo após transação: R$ {(selectedGiftCard.saldo - (parseFloat(formData.valor) || 0)).toFixed(2)}
                  </p>
                )}
              </div>
              ...
              <div className="mb-6">
                ...
                <in<response clipped><NOTE>To save on context only part of this file has been shown to you. You should retry this tool after you have searched inside the file with `grep -n` in order to find the line numbers of what you are looking for.</NOTE>